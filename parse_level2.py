...
def parse_questions(filename, output_filename):
    ...
    start_match = re.search(r'2\.CẤP ĐỘ 2', content)
    end_match = re.search(r'3\.CẤP ĐỘ 3', content)
    
    if not start_match or not end_match:
        print("Could not find start (Level 2) or end (Level 3) markers.")
        return

    section_text = content[start_match.end():end_match.start()]
    ...
    while line_idx < len(lines):
        ...
        # IGNORE standalone numbers (Chapter/Page numbers)
        if re.match(r'^\d+$', line):
            continue

        # Check for Question start
        # 1. Normal: "1. Question..."
        # 2. Embedded: "... 6. Question..."
        ...
        if start_q_match:
             # Check if this looks like a valid next question number? 
             # Or just trust it. Level 2 starts at 1?
             # Let's trust it for now unless it causes chaos.
             is_new_q = True
...
def check_embedded_start_option(q):
    # Pattern: Space + A. + Text at end of line
    # Be careful not to match "U.S.A." or meaningful text.
    # Usually strictly " A. Text..."
...
def process_content_line(q, line):
    ...
    # Relax punctuation check to allow "B.Text" (missing space)
    ...
    # If the first match starts inside the line (not 0), preamble belongs to previous context
    # Note: re.finditer includes the space in the match range if using \s+
    # We need accurate cut points.
    
    # Adjust logic: Loop through matches
    prev_end = 0
    
    # Check content BEFORE first match
    first_match_start = matches[0].start()
    
    # If check space: " A." -> start is 0 (space is at 0).
    # If "Text A." -> start is > 0.
    
    # Actually, simpler: just slice using match.start() and match.end()
...
def write_output(questions, output_filename):
    output_lines = []
    output_lines.append("# CẤP ĐỘ 2 - NHÓM CÂU HỎI TRUNG BÌNH")
    output_lines.append("")
...
parse_questions(r'e:\DeCuongPhapLuat\DeCuongPhapLuat.md', r'e:\DeCuongPhapLuat\CauHoi_CapDo2.md')
